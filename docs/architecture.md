...
## Why Bosun 410 EXIT_NOW aborts the grind

The Bosun orchestrator owns the lifecycle of every grind session it tracks. After PR #1581 its heartbeat endpoint returns HTTP 410 aggressively whenever the session is no longer in `active`/`paused` (status-flip race + recency window). The Bosun CLI surfaces that by printing `EXIT_NOW` on stderr and exiting 1 whenever `bosun grind heartbeat` sees the 410. A taskgrind controller that ignored 410 would keep spawning sessions and committing under a session Bosun has already disowned — exactly the leak path the bypass investigation tracked.

The heartbeat loop runs in a backgrounded subshell, so it can't mutate the parent controller's variables directly. When it detects `EXIT_NOW` it:

1. Writes a sentinel at `$TMPDIR/taskgrind-bosun-410-$$.flag` so the parent knows the cause.
2. Logs `bosun_heartbeat_410 session=<id> action=exit_requested grace=<N>s` for post-mortems.
3. Sends `SIGTERM` to the parent controller (`$$`) and exits cleanly.

The parent's existing `SIGTERM → graceful_shutdown` trap picks up from there. `graceful_shutdown` checks for the sentinel and, when present, swaps the normal `TG_SHUTDOWN_GRACE` (120 s) for the shorter `TG_BOSUN_410_GRACE` (default 30 s) and calls `set_phase "bosun-410-exit-now"`. The short window is deliberate: Bosun has already disowned the session, so there's no point letting a healthy backend keep computing — the operator just needs it to commit/flush and exit. `cleanup()` maps the `bosun-410-exit-now` phase into `terminal_reason` so the `grind_done` summary, the status file, and the Bosun `grind done` deregistration all carry the cause. Because `graceful_shutdown` exits the process, the session loop never re-iterates, which is the enforcement the task required for "the grind controller does NOT spawn another session after a 410."

## Why supervisor mode starts as a one-shot repair lane

The supervisor/fixer surface deliberately starts with one flag on the existing status-file contract: `taskgrind --supervise <TG_STATUS_FILE path>`. A watched run already writes the repo, phase, session summary, and now its log path into that JSON file, so the supervisor does not need a separate daemon protocol. On each invocation it classifies the watched status as healthy/progressing, terminal-clean, failed, missing, or stale. Healthy/progressing states log `supervisor_observation` and exit without launching another backend; failed or stale states log `supervisor_repair_start`, run one bounded repair session in the watched repo, then log `supervisor_repair_end`.

Keeping the first slice one-shot avoids repair storms while the failure taxonomy is still small. It also preserves the same safety model as normal sessions: the repair prompt carries the watched status path, watched log path, detected reason, normal completion protocol, public-write gate, and `TG_NO_PUSH` semantics. The supervisor does not kill the watched process, delete status/lock files, or rewrite history; any repo change must come from the repair session committing through the normal workflow. Continuous polling, cooldowns, stale-lock handling, and concrete preflight/git/backend repair heuristics can layer on top of these stable observation and repair markers without changing the public entry point.
